...
def download_report_file_on_save(sender, instance: JobAssessmentResult, created, **kwargs):
    if (
        instance.status == JobAssessmentResult.Status.COMPLETED
        and instance.report_url
        and not hasattr(instance, JobAssessmentResultReportFile.job_assessment_result.field.related_query_name())
    ):
