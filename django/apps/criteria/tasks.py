...
def download_report_file_task(job_assessment_result_id: int):
    if not (
        assessment_result_qs := JobAssessmentResult.objects.select_related(fj(JobAssessmentResult.user)).filter(
            **{
                ...
                fj(JobAssessmentResultReportFile.job_assessment_result.field.related_query_name(), IsNull): True,
            }
        )
    ).exists():
        logger.warning(
    ...
    assessment_result = assessment_result_qs.first()
    file_name = f"cpj_{assessment_result.user.username}_assessment-{assessment_result.order_id}.pdf"

    try:
        file = download_report_file(assessment_result.report_link, file_name)
        assessment_result_qs.update(**{fj(JobAssessmentResult.report_file): file})
